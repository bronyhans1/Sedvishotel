...
export function buildNightAuditCsv(audit: NightAudit): string {
  const lines = [
    ...
    "",
    "VAT",
    `VAT Collected,${audit.vatCollected}`,
    `VAT Exempt Revenue,${audit.vatExemptRevenue}`,
    `VAT Override Count,${audit.vatOverrideCount}`,
    "",
    "Cash Variance",
...
export function auditToDisplaySnapshot(audit: NightAudit) {
  return {
    ...
    vatCollected: audit.vatCollected,
    vatExemptRevenue: audit.vatExemptRevenue,
    vatOverrideCount: audit.vatOverrideCount,
  };
}
