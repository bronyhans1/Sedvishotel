...
export function WalkInPageContent({ access }: WalkInPageContentProps) {
  ...
  const total =
    subtotal > 0 ? Math.round(subtotal * 1.15 - form.discount) : 0;
  const balance = Math.max(0, total - form.amountPaid);
  ...
  function canAdvance(currentStep: number): boolean {
    ...
    if (currentStep === 4) return form.amountPaid >= 0;
    return true;
  }
              ...
              <CardContent className="space-y-6 pt-6">
                ...
                {step === 4 && (
                  <>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      <WizardField label="Amount Due">
                        <Input readOnly value={total > 0 ? formatCurrency(total) : ""} />
                      </WizardField>
                      <WizardField label="Amount Paid">
                        <Input
                          type="number"
                          min={0}
                          step="0.01"
