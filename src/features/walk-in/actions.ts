...
import { mapAvailableRoomsToWalkInOptions } from "@/lib/rooms/available-room-options";
...
export async function getWalkInAvailableRoomsAction(
  ...
  try {
    ...
    const rooms = await reservationService.checkAvailability(ctx, session, {
      checkIn,
      checkOut,
    });
    return mapAvailableRoomsToWalkInOptions(rooms);
  } catch (err) {
